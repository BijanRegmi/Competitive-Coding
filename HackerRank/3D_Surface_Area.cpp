...
void solve(){
    ...
    fo(i, H){
        fo(j, W){
            cin >> x;
            heightmap[i][j] = x;
            totalsurfacearea+=(2+4*x);
        }
    }

    fo(i, H){
        fo(j, W){
            if (j<W-1) totalsurfacearea -= 2*min(heightmap[i][j+1], heightmap[i][j]);
            if (i<H-1) totalsurfacearea -= 2*min(heightmap[i+1][j], heightmap[i][j]);
        }
    }
